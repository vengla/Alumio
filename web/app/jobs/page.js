        ...
        try {
            ...
            if (res.success) {
                setJobs(prev => prev.map(j => {
                    if (j._id === jobId) {
                        if (modalJob && modalJob._id === jobId) {
                            setModalJob(res.data);
                        }
                        return res.data;
                    }
                    return j;
                        ...
                        <div className="card" style={{ width: '600px', maxHeight: '85vh', overflowY: 'auto' }}>
                            ...
                            <div className="mb-6">
                                ...
                                <ul style={{ listStyle: 'disc', paddingLeft: '1.5rem' }} className="text-muted">
                                    {modalJob.requirements && modalJob.requirements.length > 0 ? (
                            ...
                            {modalJob.experience && (
                                <div className="mb-6">
                                    <h4 className="h4 mb-2">Experience Needed</h4>
                                    <p className="text-muted">
                                        {typeof modalJob.experience === 'object'
                                            ? `${modalJob.experience.min || 0} - ${modalJob.experience.max || 'Any'} years`
                                            : modalJob.experience}
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="flex justify-end gap-3 mt-4 pt-4 border-t">
                                ...
                                {user.role !== 'admin' && (
                                    modalJob.applicants?.some(a => a.userId === user.id) ? (
                                        <button
                                            className="btn"
                                            style={{ background: '#fee2e2', color: '#ef4444', border: '1px solid #ef4444' }}
                                            onClick={() => handleApply(modalJob._id, true)}
                                        >
                                            Cancel Application
                                        </button>
                                    ) : (
